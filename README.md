...
- **Ambient Intent Buffer** — per-session signal accumulation that surfaces scenario, decision, and sprint materialization offers at the optimal briefing moment; five archetypes (deadline, opportunity, pattern, conflict, goal_drift) each with configurable thresholds; idempotent writes with pre-write snapshot and audit trail
...
- **Morning digest brief** — automated lightweight briefing at 07:00 Mon–Fri via `python scripts/pipeline.py --mode=digest`; includes P1 open items, goal progress, calendar snapshot, and health summary; launchd job defined in `scripts/com.artha.morning-brief.plist` (copy to `~/Library/LaunchAgents/` and `launchctl load` to activate)
- **Citation enforcement (F-D2)** — briefing items citing open items, state domains, or signals include machine-readable tokens (`[src: OI-NNN]`, `[src: state:<domain>]`, `[src: signal:SIG-NNN]`); `python scripts/pipeline_audit.py --last` audits the most recent briefing for uncited factual-verb sentences and writes a one-line summary to `state/audit.md`
