...
- **Autonomous skills** — property tax, visa bulletin, passport expiry, vehicle recalls, subscription lifecycle monitor, financial resilience, **home device monitor**, coaching nudges. Skills now carry **per-run health counters** in `state/skills_cache.json` (broken/degraded/healthy classifications); `/eval skills` shows the health table; R7 adaptive cadence auto-reduces check frequency for skills returning empty data. `/eval effectiveness` shows engagement rate trends across catch-ups.
