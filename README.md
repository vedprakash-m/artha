...
- **OpenClaw Home Bridge** — HMAC-secured M2M channel to Home Assistant; Artha context drives TTS announcements and WhatsApp-gated drafts; home presence and energy events flow back into briefings (opt-in, feature-flagged)
- **Windows Always-On Mode (ACI)** — `artha_engine.py` singleton hosts the Telegram bot (workout logging, watch alerts), daily 7am PT pipeline schedule, and 30-min health watchdog. Bidirectional bridge to OpenClaw for brief requests and domain queries. See [Tech Spec §34](specs/artha-tech-spec.md) for architecture.
