    ...
    *,
    ...
    try:
        ...
        for chunk_start in range(0, len(all_uids), _CHUNK_SIZE):
            ...
            # BODY.PEEK[] is used instead of RFC822 — iCloud returns no body for
            # RFC822 fetch but responds correctly to BODY.PEEK[] (non-marking).
            status, fetch_data = mail.uid("fetch", uid_list, "(BODY.PEEK[])")  # type: ignore[call-overload]
            if status != "OK" or not fetch_data:
                continue

            # fetch_data alternates: (b'UID BODY[] {size}', b'raw bytes'), b')'
